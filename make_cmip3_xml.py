...
PJD 15 Aug 2024     - hacked to get working with py3

@author: durack1
"""

import os, datetime, time, glob, gc, subprocess, argparse, re
import pdb
...
parser.add_argument(
    ...
    metavar="str",
    type=str,
    help="including 'makefiles' as a command line argument will ensure files are written",
)
...
host_path = "xmls/"
cdat_path = "/home/durack1/mambaforge/envs/xcd061cdmcdu/bin/"
...
logfile = os.path.join(host_path, "".join([time_format, "_make_cmip3_xml-", ".log"]))

# Create data pathnames
path_data = "/p/css03/esgf_publish/cmip3/ipcc/"
...
experiments.sort()
temporal = ["mo", "fixed"]
# For months and fixed fields only
atm_vars = ["pr", "tas"]
atm_vars.sort()
atm_vars2 = [
    "sci",
...
fx_vars.sort()
land_vars = ["mrro", "mrros"]
land_vars.sort()
ocn_vars = ["so", "sos", "thetao", "tos", "wfo", "zos", "rhopoto", "uo", "vo", "wo"]
ocn_vars.sort()
seaIce_vars = ["sic", "sit"]
seaIce_vars.sort()
len_vars = (
...
# Create length counter for reporting
...
i2 = 0
exclude_paths = ["cam3.3", "cmip5", "T4031qt", "summer", "aggregate"]
list_data_outfiles = []
list_data_outfiles_paths = []
for path in list_data_paths:
    # print("path:", path)
    path_bits = path.split("/")
    # print(i2,path_bits)
    if path_bits[6] in exclude_paths:
        continue
    elif "data" in path_bits[6]:
        model = path_bits[11]
        experiment = path_bits[7]
        time_ax = path_bits[9]
        realisation = path_bits[12]
        version = "2"
        variable = path_bits[10]
    else:
        # print(i2,path_bits)
        model = path_bits[10]
        experiment = path_bits[6]
        time_ax = path_bits[7]
        realisation = path_bits[11]
        version = "1"
        variable = path_bits[9]
    # Test for list entry and trim experiments and variables to manageable list
    if (
        ((experiment == "20c3m") or (experiment == "amip"))
        and (time_ax in temporal)
        and ((variable in ocn_vars) or (variable in atm_vars2))
    ):
        #                                      cmip3.bccr_bcm2_0.sresa1b.run1.mo.pr.xml
        list_data_outfiles.insert(
            i2,
            "".join(
                [
                    "cmip3.",
        ...
        and (time_ax in temporal)
        and (
            (variable in ocn_vars)
            or (variable in atm_vars)
            or (variable in seaIce_vars)
            or (variable in land_vars)
            or (variable in fx_vars)
        )
    ):
        list_data_outfiles.insert(
            i2,
...
print(
    "'ipcc' scan complete.. %d paths total; %d output files to be written (%d vars sampled)"
    % (i1, len(list_data_outfiles), len_vars)
)
logfile_handle.write(
    "".join(
        [
            ...
            format(i1, "1d"),
            " paths total; ",
            format(len(list_data_outfiles), "1d"),
...
gc.collect()
time.sleep(1)
# Pause and acknowledge "data" success

# Sort lists by outfiles
outfilesAndPaths = list(zip(list_data_outfiles, list_data_outfiles_paths))
outfilesAndPaths.sort()
# sort by str value forgetting case - key=str.lower; requires str object
del (list_data_outfiles, list_data_outfiles_paths)
gc.collect()
list_outfiles, list_outfiles_paths = list(zip(*outfilesAndPaths))
...
deque(
    (list.pop(list_outfiles_paths, i) for i in sorted(purge_index, reverse=True)),
    maxlen=0,
)
del purge_index
gc.collect()
...
if make_xml:
    ...
    # xml_count1 = o.read()
    command = "".join(["ls ", host_path, "*/*/mo/*/*.xml | wc -l"])
    ...
    print(
        "".join(["** Updating ", format(xml_count, "1d"), " existing *.xml files **"])
    )
    logfile_handle = open(logfile, "a")
    logfile_handle.write(
        "".join(["** Updating ", format(xml_count, "1d"), " existing *.xml files **\n"])
    ...
    """
    # os.popen3 splits results into input, output and error - consider subprocess function in future
    ...
    for paths in list_outfiles_paths:
        ...
        if temporal == "mo":
            # temporal = "mo_new"
            temporal = "mon"
            # Updated path; existing xmls are in place until successful xml write completion
        elif temporal == "fixed":
            # temporal = "fx_new"
            temporal = "fx"
        ...
        # if os.path.isfile
        # Generate xml file
        # check for *.nc.bad data
        testList = glob.glob(os.path.join("".join(list_outfiles_paths[i]), "*.nc"))
        print("scanning path:", os.path.join("".join(list_outfiles_paths[i]), "*.nc"))
        ...
        cmd = "".join(
            [
                ...
                outfilename,
                " ",
                os.path.join("".join(list_outfiles_paths[i]), "*.nc"),
            ]
        )
        ...
        if fileerror and zerofiles:
            ...
            # Case cdscan call reports an error - print "".join(ln) ; # Print error
            err_text = "".join(
                [" PROBLEM 2 (cdscan error - '", errorcode, "') creating "]
            )
            logfile_handle.write(
                "".join(
                    [
    ...
    print(
        "".join(
            [
                "** Complete for 'ipcc' sources; Total outfiles: ",
                format(len(list_outfiles), "01d"),
                " **",
            ]
        )
    )
    ...
    print(
        "".join(
            [
                "** XML file count - Bad/skipped: ",
                format(xml_bad - 3, "1d"),
                "; bad1 (cdscan - zero files): ",
                format(xml_bad1 - 1, "1d"),
                "; bad2 (cdscan - warning specified): ",
                format(xml_bad2 - 1, "1d"),
                "; bad3 (no outfile): ",
                format(xml_bad3 - 1, "1d"),
            ]
        )
    )
    logfile_handle = open(logfile, "a")
    logfile_handle.write(
        "".join(
            [
                "** make_cmip3_xml.py complete for 'ipcc' sources; Total outfiles: ",
                format(len(list_outfiles), "01d"),
                " **\n",
            ]
        )
    ...
    logfile_handle.write(
        "".join(["** XML file count - Good: ", format(xml_good - 1, "1d"), " **\n"])
    )
    logfile_handle.write(
        "".join(
            [
    ...
    print(
        "".join(
            [
                "** Archive and migration complete from */*/*_new to */*/*, archive file: ",
                host_path,
                "_archive/",
                time_format,
                "_cmip3_xml.7z **",
    ...
    logfile_handle.write(
        "".join(
            [
                "** Archive and migration complete from */*/*_new to */*/*,\n archive file: ",
                host_path,
                "_archive/",
                time_format,
                "_cmip3_xml.7z **",
